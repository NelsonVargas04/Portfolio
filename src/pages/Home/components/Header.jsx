...
export default function Header({ nav, lang, onSwitchLang, theme, onToggleTheme }) {
  ...
  return (
    <nav className={scrolled ? 'scrolled' : ''}>
      <a href="#hero" className="nav-logo">NV_</a>
      <div className="nav-links">
        <a href="#about"      className="nav-link">{nav.about}</a>
        <a href="#experience" className="nav-link">{nav.experience}</a>
        <a href="#projects"   className="nav-link">{nav.projects}</a>
        <a href="#contact"    className="nav-link">{nav.contact}</a>
      </div>
      <div className="nav-lang">
        <button className={`lang-btn${lang === 'es' ? ' active' : ''}`} onClick={() => onSwitchLang('es')}>ES</button>
